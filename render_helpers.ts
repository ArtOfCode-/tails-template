 ...
 *             or an object or array to render as JSON
...
export const render = async (req: et.Request, res: ResponseWithLayout, view: string | object | Array<any>, locals: Object = {},
    { layout, pool, status }: { layout?: string | false, pool: mt.Pool, status?: number }) => {
    if (status) {
        res.status(status);
    }
    ...
    if (typeof (view) === 'string') {
        ...
        if (pool) {
            locals['current_user'] = await getUser(req, res, pool); // eslint-disable-line no-use-before-define
        }
        else {
        ...
        locals = Object.assign(locals, viewHelpers(req, res, pool), { req, res });
        if (layout === false) {
