...
export default (req: et.Request, res: ResponseWithLayout, pool: mt.Pool) => {
    ...
    /**
     * Sanitize raw HTML for use in a view.
     * @param raw the dirty HTML string to sanitize
     * @param options options to be passed to sanitize-html
     */
    const sanitize = (raw: string, options?: {}): string => {
        return sanitizeHtml(raw, Object.assign({
