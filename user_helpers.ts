...
const debug = createDebug('app:user_helpers');
 ...
 *          doesn't reject, ever
 */
export const getUser = async (req: et.Request, res: ResponseWithLayout, pool: mt.Pool): Promise<User | null> => {
    ...
    return new Promise<User>(async resolve => {
        const cookieName = config.getSiteSetting('sessionCookieName');
        if (req.cookies[cookieName]) {
            const key = await getSecretKey(pool);
            let userData;
            try {
                userData = JSON.parse(decrypt(req.cookies[cookieName], key));
            }
            catch (err) {
                debug('Failed to decrypt user cookie; clearing.');
...
export const setUser = async (res: et.Response, id: number, pool: mt.Pool): Promise<void> => {
    ...
    res.cookie(config.getSiteSetting('sessionCookieName'), cookieData, { maxAge: 2678400000, httpOnly: true });
};
 ...
 */
export const requireAuth = async (req: et.Request, res: ResponseWithLayout, pool: mt.Pool): Promise<User | null> => {
    const user = await getUser(req, res, pool);
    if (!user) {
...
/**
 * Initiates authentication with Stack Exchange by redirecting the response to the Stack Exchange authentication page.
 * @param res the response object
 * @param returnUrl an optional URI to return to once authentication is finished
 */
export const initiateStackAuth = (res: ResponseWithLayout, returnUrl?: string) => {
    res.redirect(`https://stackexchange.com/oauth?client_id=${config.getSiteSetting('clientId')}&scope=&state=${returnUrl}&redirect_uri=${config.getSiteSetting('redirectUri')}`);
};
